...
class _PendingVersion extends \IPS\Content\Item implements \IPS\Content\Hideable
{
	...
	{
		$ids = [];
		foreach( \IPS\Db::i()->select( '*', 'downloads_files_records', array( 'record_file_id=? AND record_hidden=1', $this->file_id ) ) as $record )
		{
				...
				case 'ssupload':
						...
						try
						{
							if( $record['record_type'] == 'ssupload' AND !empty( $record['record_thumb'] )
								AND !\IPS\Db::i()->select( 'COUNT(record_id)', 'downloads_files_records', array( 'record_id<>? AND record_thumb=?', $record['record_id'], $record['record_thumb'] ) )->first() )
							{
