	...
	protected function _manageRobotstxt()
	{
		...
		if ( isset( \IPS\Request::i()->_saved ) )
		{
			...
			if ( $dir )
			{
				...
				{
					$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack( 'use_robotstxt_warning_off', NULL, ['sprintf' => [$rootUrl]] ), 'ipsMessage ipsMessage_warning' );
				}
				else
				{
					$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack( 'use_robotstxt_warning_download', NULL, ['sprintf' => [$rootUrl]] ), 'ipsMessage ipsMessage_warning' );
				}
			}
			else if ( ! \IPS\CIC2 AND file_exists( \IPS\ROOT_PATH . '/robots.txt' ) )
			{
				if ( \IPS\Settings::i()->robots_txt === 'off' )
				{
					$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack( 'use_robotstxt_warning_existing_off', NULL, ['sprintf' => [ rtrim( \IPS\Http\Url::baseUrl(), '/' ) ] ] ), 'ipsMessage ipsMessage_warning' );
				}
				else
				{
					$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack( 'use_robotstxt_warning_existing_download', NULL, ['sprintf' => [ rtrim( \IPS\Http\Url::baseUrl(), '/' ) ] ] ), 'ipsMessage ipsMessage_warning' );
				}
			}
		}
