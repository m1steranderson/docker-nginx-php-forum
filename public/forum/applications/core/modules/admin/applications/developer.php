...
class _developer extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageAcpmenu()
	{
		...
		if ( \IPS\Request::i()->do === 'reorder' )
		{
			...
			{
				if ( $parent === NULL OR array_key_exists( $parent, $_menu ) )
				{
					$menu[ $parent ] = array();
	 ...
	 * @return	string
	 */
	protected function _manageFeatures()
	{
		if ( !\in_array( $this->application->directory, \IPS\IPS::$ipsApps ) )
		{
			\IPS\Output::i()->error( 'app_ips_only', '2C103/R', 403, '' );
		}
		
		return \IPS\Application\Feature::devTable(
			\IPS\Http\Url::internal( "app=core&module=applications&controller=developer&appKey={$this->application->directory}&tab=features" ),
			$this->application->directory
		);
	}
