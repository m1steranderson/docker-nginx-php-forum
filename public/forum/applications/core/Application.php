...
class _Application extends \IPS\Application
{
	...
	public function doMemberCheck(): ?\IPS\Http\Url
	{
		...
		if( ( \IPS\Member::loggedIn()->real_name === '' or !\IPS\Member::loggedIn()->email ) and \IPS\Dispatcher::i()->controller !== 'register' and \IPS\Dispatcher::i()->controller !== 'login' )
		{
		...
		elseif(
			...
			\IPS\Dispatcher::i()->controller !== 'checkout'
		)
		{
		...
		/* Have required profile actions that need completing */
			...
			\IPS\Settings::i()->quick_register AND
			!\IPS\Member::loggedIn()->members_bitoptions['profile_completed'] AND
			!\in_array( \IPS\Dispatcher::i()->controller, array( 'register', 'login', 'redirect', 'ajax', 'settings', 'checkout', 'pixabay', 'editor' ) ) AND
		...
		/* Need to set up MFA... */
		if ( !\in_array( \IPS\Dispatcher::i()->controller, array( 'register', 'login', 'redirect', 'ajax', 'settings', 'embed' ) ) )
		{
		...
		if ( \IPS\Dispatcher::i()->controller !== 'settings' AND ( !isset( \IPS\Request::i()->area ) OR \IPS\Request::i()->area !== 'password' ) )
		{
