...
class _Achievements
{
	...
	public function parse_new_badge( \IPS\Notification\Inline $notification, $htmlEscape=TRUE )
	{
		if ( $notification->extra )
		{
			...
			try
			{
				...
				$content = \IPS\Member::loggedIn()->language()->addToStack( 'notification__new_badge_content', NULL, [ 'sprintf' => [ $badge->_title ] ] );
				$title = \IPS\Member::loggedIn()->language()->addToStack( 'notification__new_badge', NULL, [ 'sprintf' => [ $badge->_title ] ] );
			}
			catch( \Exception $e )
			{
				throw new \OutOfRangeException;
	...
	{
		return array(
			'body'		=> $language->addToStack( 'notification__new_badge', NULL, [ 'sprintf' => [ $badge->_title ] ] ),
			'data'		=> array(
