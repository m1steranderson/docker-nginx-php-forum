	...
	public function edit()
	{
		$wizard = new \IPS\Helpers\Wizard( array(
			'google_maps_enable_apis'		=> function( $data )
			{
				...
				$form->add( new \IPS\Helpers\Form\YesNo( 'googlemaps', isset( $data['googlemaps'] ) ? $data['googlemaps'] : \IPS\Settings::i()->googlemaps, FALSE, array( 'togglesOn' => array( 'googleApi_jsapi', 'googleApi_staticapi', 'googleApi_geocodeapi' ) ) ) );
				...
				{
					if ( $values['googlemaps'] or $values['googleplacesautocomplete'] )
					{
						return $values;
					}
					else
