...
class _category extends \IPS\cms\Databases\Controller
{
	...
	public function view()
	{
		...
		\IPS\Member::loggedIn()->language()->words['cms_record_i_started'] = \IPS\Member::loggedIn()->language()->addToStack( 'cms_record_i_started_sprintf', NULL, array( 'sprintf' => $database->recordWord() ) );
		...
		if ( $cookie !== NULL )
		{
			foreach( $cookie as $f => $v )
			{
				if ( $f == 'cms_record_i_started' and \IPS\Member::loggedIn()->member_id )
				{
					...
					\IPS\Member::loggedIn()->language()->words['content_field_cms_record_i_started'] = \IPS\Member::loggedIn()->language()->addToStack( 'cms_record_i_started_sprintf', NULL, array( 'sprintf' => $database->recordWord() ) );
