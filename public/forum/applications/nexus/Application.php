...
class _Application extends \IPS\Application
{
	...
	{
		/* Disable hosting support for new installations */
		\IPS\Db::i()->update( 'core_modules', array( 'sys_module_visible' => 0 ), array( 'sys_module_application=? AND sys_module_key=? AND sys_module_area=?', 'nexus', 'hosting', 'admin' ) );
	}
	...
	public function acpMenu()
	{
		...
		if( $m = \IPS\Application\Module::get( 'nexus', 'hosting', 'admin' ) AND $m->visible )
		{
			$menu['hosting'] = array(
				'queues' => array(
					'tab'	=> 'nexus',
					'controller'	=> 'queues',
					'do'	=> '',
					'restriction'	=> 'queues_manage'
				),
				'servers'	=> array(
					'tab'	=> 'nexus',
					'controller'	=> 'servers',
					'do'	=> '',
					'restriction'	=> 'servers_manage'
				),
				'monitoring'	=> array(
					'tab'	=> 'nexus',
	...
	{
		/* @todo 4.6.12 introduced a change to abstract some dispatcher logic to let apps handle it themselves
		   		this Nexus code was moved to the new method which changed how the code is triggered. This meant
				that members registered before the admin upgraded are now being asked to purchase a package when
				before they weren't.
				We are working on a more elegant fix, but this fix simply ignores this bit of logic for all members registered
				before 4.6.12 was released */
		if ( \IPS\Member::loggedIn()->joined->getTimestamp() > 1649161869 )
		{
			if ( \IPS\Settings::i()->nexus_subs_register AND !\in_array( \IPS\Dispatcher::i()->module->key, array( 'store', 'checkout', 'subscriptions' ) ) )
			{
				if ( !\IPS\nexus\Subscription::loadByMember( \IPS\Member::loggedIn(), TRUE ) )
				{
					return \IPS\Http\Url::internal( "app=nexus&module=subscriptions&controller=subscriptions&register=1", 'front', 'nexus_subscriptions' );
				}
			}
			
			if ( \IPS\Settings::i()->nexus_reg_force AND !\in_array( \IPS\Dispatcher::i()->module->key, array( 'store', 'checkout', 'subscriptions' ) ) )
			{
				if ( !\IPS\Db::i()->select( 'COUNT(*)', 'nexus_purchases', array( "ps_member=? AND ps_active=?", \IPS\Member::loggedIn()->member_id, 1 ) )->first() )
				{
	...
	/**
	 * Get Extensions - Deprecate Hosting Utilities
	 *
	...
	{
		$extensions = parent::extensions( $app, $extension, $construct, $checkAccess );

		/* If Item extension and hosting support is hidden, we'll need to remove the hosting specific items that are deprecated */
		if( $extension == 'Item' AND $m = \IPS\Application\Module::get( 'nexus', 'hosting', 'admin' ) AND !$m->visible )
		{
			$deprecated = array( 'Bandwidth', 'DedicatedServer', 'Domain' );
