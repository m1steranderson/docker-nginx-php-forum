...
class _languages extends \IPS\Node\Controller
{
	...
	protected function download()
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'language_export_author_url', $lang->author_url, false ) );
		$form->add( new \IPS\Helpers\Form\Text( 'language_export_update_check', $lang->update_url, false ) );
		...
		if ( $values = $form->values() )
		{
			...
			$lang->author_url		= $values['language_export_author_url'];
			$lang->update_url		= $values['language_export_update_check'];
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			...
			function( $data )
			{
				...
				while ( $xml->read() )
				{
					...
					if ( !array_key_exists( $appKey, $data['apps'] ) AND \in_array( $appKey, $applications ) )
					{
						/* Get version */
