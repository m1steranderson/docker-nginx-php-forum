...
class _cleanup extends \IPS\Task
{
	...
	public function execute()
	{
		...
		/* Turn on things for cloud 2 */
		if ( \IPS\CIC2 AND \IPS\Cicloud\getForcedArchiving() )
		{
			\IPS\Db::i()->update( 'core_tasks', array( 'enabled' => 1 ), array( '`key`=?', 'archive' ) );

			/* 12 years is the cutoff if archiving is forced, so if it's over 12 years, drop the value down */
			if( \IPS\Settings::i()->archive_last_post_cloud > 12 )
			{
				\IPS\Settings::i()->changeValues( array( 'archive_last_post_cloud' => 12 ) );
			}
		}
